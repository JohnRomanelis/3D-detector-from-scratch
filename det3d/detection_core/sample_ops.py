...
def shape_mergeable(x, expected_shape):
    ...
    if isinstance(x, (list, tuple, np.ndarray)) and \
            isinstance(expected_shape, (list, tuple, np.ndarray)):
        x = np.array(x)
...
class DataBaseSampler:
    ...
    def __init__(self, 
        ...
        #TODO: Implement dbprepor -> Done
        # Filtering Info Files
    ...
    def sample_all(self,
        ...
        for class_name, max_sample_num in zip(self._sample_classes,
            ...
            samlpled_num = np.round(self._rate * sampled_num).astype(np.int64)
            sampled_num_dict[class_name] = sampled_num
            sample_num_per_class.append(samlpled_num)
        ...
        # So far we have found the ground truths that need to be sampled 
        # Now we need to load the point cloud from the actual files
        if len(sampled) > 0:
            sampled_gt_boxes = np.concatenate(sampled_gt_boxes, axis=0)
            num_sampled = len(sampled)
            s_points_list = []
            for info in sampled:
                # loading point cloud points
            ...
            ''' # Currently no random crop is used
            if random_crop:
                s_points_list_new = []
                ...
                crop_frustrum = prep.random_crop_frustrum(
    ...
    def sample_class_v2(self, name, num, gt_boxes):
        ...
        boxes = np.concatenate([gt_boxes, sp_boxes]).copy()
