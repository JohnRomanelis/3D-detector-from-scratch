...
def create_target_np(all_anchors,
    ...
    ## ---------- Anchor Filtering ---------- ##
    # filtering anchors based on the pruning function
    if prune_anchor_fn is not None:
        # get inds of the anchors to keep
        inds_inside = prune_anchor_fn(all_anchors)
        # keeping specified anchors
    ...
    # calculating the number of anchors after pruning
    num_inside = len(inds_inside) if inds_inside is not None else total_anchors

    ## ---------- Initialization ---------- ##
    # number of parameters for each ground truth bounding box
    # e.g. bounding box center, dimensions, rotation etc
    box_ndim = all_anchors.shape[1]

    # Add default values if gt_importance or gt_class is not provided for the bboxes
    ...
    # labels=1 is positive, 0 is negative, -1 is don't care (ignore)
    ...
    ## ---------- Assigning Targets ---------- ##
    # if ground truth boxes exist in the scene
    if len(gt_boxes) > 0:
        # Compute overlaps between the anchors and the gt boxes
        # expected output: [num_anchors x num_gt_boxes]
        anchor_by_gt_overlap = similarity_fn(anchors, gt_boxes)
        # Map each anchor to the bounding box with which it has
        # the highest overlap
        anchor_to_gt_argmax = anchor_by_gt_overlap.argmax(axis=1)
        # for each anchor, get the value of the max overlap
        # NOTE: np.arange(num_inside): all indexes (should be the same as : )
        anchor_to_gt_max = anchor_by_gt_overlap[np.arange(num_inside), 
                                                anchor_to_gt_argmax]
        # Map each bounding box to the anchor with which it has
        # the highest overlap
        gt_to_anchor_argmax = anchor_by_gt_overlap.argmax(axis=0)
        # for each gt get the value of the max overlap
    ...
    if inds_inside is not None:
        print("Using unmap function (inside v3d/core/torget_ops")
        print("(comment out this line if knowingly use unmap)")
    ...
    if inds_inside is not None: 
        ret["assigned_anchor_inds"] = inds_inside[fg_inds]
    else:
        ret["assigned_anchor_inds"] = fg_inds
